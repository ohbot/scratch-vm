...
const editorExtensionId = 'naiphpcboecoakmocdpndbhjmkggaedc';
...
class Scratch3OhbotBlocks {
    ...
    async setMotorPosition(args) {
        unityInstance.SendMessage(
            Robot,
            'M' + args.MOTOR,
            parseFloat(args.POSITION)
        );
        await this.runCommand(['MM', args.MOTOR, args.POSITION]);
    }
    async changeMotorPosition(args) {
        unityInstance.SendMessage(
            Robot,
            'C' + args.MOTOR,
            parseFloat(args.POSITION)
        );
        await this.runCommand(['MC', args.MOTOR, args.POSITION]);
    }
    async setMotorSpeed(args) {
        await this.runCommand(['MS', args.MOTOR, args.SPEED]);
    }
    async setNamedColour(args) {
        unityInstance.SendMessage(Robot, 'SetColByName', args.COLOURNAME);
        await this.runCommand(['CC', args.COLOURNAME]);
    }

    async setEyeShape(args) {
        if (Robot == 'Picoh') {
            unityInstance.SendMessage(Robot, 'SetEyeShape', args.EYESHAPE);
        }
        await this.runCommand(['ES', args.EYESHAPE]);
    }

    async setRGBColour(args) {
        if (args.RGB == 'red') {
            unityInstance.SendMessage(
                Robot,
                'SetR',
                parseFloat(args.RGBCOLOUR)
            );
        }
        if (args.RGB == 'green') {
            unityInstance.SendMessage(
        ...
        await this.runCommand(['CE', args.RGB, args.RGBCOLOUR]);
    }
    ...
    runCommand(cmd) {
        return new Promise((resolve, reject) => {
            ...
            chrome.runtime.sendMessage(editorExtensionId, cmd, (response) => {
                if (chrome.runtime.lastError) {
                    console.warn('Extension error:', chrome.runtime.lastError.message);
                    return resolve(); // don't block even on failure
                }
                requestAnimationFrame(() => resolve(response));
            });
        });
    }
    ...
    oh_mouseX() {
        ...
        return (
            Math.round(
                (this.runtime.ioDevices.mouse.getScratchX() + 240) / 4.8
            ) / 10
        );
    }
    ...
    oh_mouseY() {
        ...
        return (
            Math.round(
                (this.runtime.ioDevices.mouse.getScratchY() + 180) / 3.6
            ) / 10
        );
    }
                ...
                (err, res, body) => {
                    ...
                    audioEngine.audioContext
                        ...
                        .then(function (overallMax) {
                            audioProcessor =
                                audioEngine.audioContext.createScriptProcessor(
                                    0,
                                    1,
                                    1
                                );
                            audioProcessor.onaudioprocess = function (event) {
                                ...
                                var volume =
                                    LIP_MIN +
                                    (sampleMax / overallMax) *
                                        (LIP_MAX - LIP_MIN);
                                instance._lip =
                                    Math.round(
                                        (volume + Number.EPSILON) * 100
                                    ) / 100;
                            };
